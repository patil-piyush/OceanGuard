...
export default function DebrisDetection() {
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      setResult({
        debrisType: "Plastic Waste",
        confidence: 92,
        severity: "High",
        estimatedArea: "250 m²",
        coordinates: useCurrentLocation ? location : location,
        recommendations: [
          "Alert Coast Guard immediately",
          "Deploy cleanup team within 2 hours",
  ...
  const handleReportToAuthority = () => {
    setSubmitted(true)
    setTimeout(() => {
      alert("Report submitted to nearby authorities!")
      setSubmitted(false)
      setResult(null)
      setImage(null)
      setPreview("")
  ...
  return (
    <div className="space-y-6 py-8">
      <div className="space-y-2">
        <h1 className="text-3xl md:text-4xl font-bold">Debris Detection</h1>
        <p className="text-muted-foreground">Upload satellite or UAV imagery to detect and classify marine debris</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <Card className="border-border/50 bg-card md:col-span-1 h-fit">
          <CardHeader>
            ...
            <CardDescription>Select satellite or UAV image for analysis</CardDescription>
          </CardHeader>
        ...
        <Card className="border-border/50 bg-card md:col-span-1">
          <CardHeader>
            ...
            <CardDescription>AI-powered detection and classification</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {result ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-background/50 rounded-lg p-3 space-y-1">
                    <p className="text-xs text-muted-foreground">Debris Type</p>
                    <p className="font-semibold text-sm">{result.debrisType}</p>
                  </div>
                  <div className="bg-background/50 rounded-lg p-3 space-y-1">
                  ...
                  <div className="bg-background/50 rounded-lg p-3 space-y-1">
                    ...
                    <p
                      className={`font-semibold text-sm ${result.severity === "High" ? "text-red-500" : "text-yellow-500"}`}
                    >
                      {result.severity}
                    </p>
                  </div>
                  <div className="bg-background/50 rounded-lg p-3 space-y-1">
                    ...
                    <p className="font-semibold text-sm">{result.estimatedArea}</p>
                  </div>
                </div>
                ...
                <Button
                  ...
                  className="w-full bg-accent hover:bg-accent/90 gap-2"
                >
                  {submitted ? "Reporting..." : "Report to Authority"}

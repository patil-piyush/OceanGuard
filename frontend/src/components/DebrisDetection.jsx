...
export default function DebrisDetection({ userRole }) {
  // debris type option removed — detection will auto-identify type
  ...
  const handleGetCurrentLocation = () => {
    ...
    // prefer GPS but fallback to IP-based location if GPS fails (unless permission denied)
    import("../utils/geolocation")
      ...
      .then((result) => {
        ...
        if (result._fallbackNotice) {
          // Inform the user that a fallback was used
          alert("Location obtained via IP-based fallback (less accurate).");
        }
      })
      .catch((err) => {
        // Friendly error messages for common geolocation errors
  ...
  const handleAnalyze = async (e) => {
    ...
    // Simulate API call for debris analysis
    setTimeout(() => {
      ...
      // auto-detect debris type (mocked)
      const possibleTypes = [
        "Plastic Microbeads",
      ...
      // Add to history (store a short type label)
    ...
    <div className="detection-container">
      <div className="detection-layout">
        ...
        <div className="detection-panel">
          <h2>Debris Detection</h2>

          <form onSubmit={handleAnalyze} className="detection-form">
            {/* Debris type selection removed — analysis will auto-detect debris type */}
